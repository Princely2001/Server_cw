...
class Bidding_model extends CI_Model {
    ...
    private $api_logs_table = 'api_logs';
    ...
    public function get_resolved_winner_with_user($target_date) {
        return $this->db->select('b.id AS bid_id, b.user_id, b.bid_amount, b.target_date, u.first_name, u.last_name, u.university_email')
    ...
    public function get_losers_with_user($target_date) {
        return $this->db->select('b.id AS bid_id, b.user_id, b.target_date, u.first_name, u.last_name, u.university_email')
                        ->from($this->bids_table . ' b')
                        ->join($this->users_table . ' u', 'u.id = b.user_id')
                        ->where('b.target_date', $target_date)
                        ->where('b.status', 'lost')
    ...
    public function log_api_access($api_key_id, $endpoint, $ip_address) {
        return $this->db->insert($this->api_logs_table, [
